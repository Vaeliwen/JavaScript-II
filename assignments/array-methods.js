...
function unlimitedPower(item) {
    let all_caps = item.first_name.toUpperCase();
    return all_caps;
}

let allCaps = runners.map(unlimitedPower)
//console.log(allCaps); 
...
// Problem 1
//Need to alphabetize the runners by last name, and return just those to the director.

function getLastNames(item) {
    return item.last_name;
}

let lastNames = runners.map(getLastNames).sort()

//console.log(lastNames)
// Problem 2

//I'm really having trouble seeing how this challenge is going to assist me, as there really isn't that much to do with this dataset that I haven't already done with either today's challenge, or similarly with yesterday's.
